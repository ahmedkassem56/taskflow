...
def _apply_item_patch(db_conn: sqlite3.Connection, row, patch) -> tuple[dict, dict | None]:
    ...
    *row* is the pre-update items row (already locked via BEGIN IMMEDIATE).
    Returns (item_dict, spawned_dict|None). Raises HTTPException for 422/409.
    """
    ...
    if provided == {"done"} and patch.done is True and already_done:
        return db.item_from_row(row), None
    ...
    # Spawn: done flips 0 -> 1 on a recurring item (§2.5).
    spawned = None
    if row["done"] == 0 and merged_done == 1 and merged_recurrence != "none":
        ...
        cur = db_conn.execute(
            ...
            (
                ...
                merged("priority"),
                next_date.isoformat(),
                _store_quantity(merged("quantity")),
                merged_recurrence,
                merged_interval,
                now,
                now,
        ...
        spawned = db.fetch_item(db_conn, cur.lastrowid)
...
def create_app() -> FastAPI:
    ...
    def create_item(body: ItemCreate, _db: sqlite3.Connection = Depends(get_db)):
        ...
        return db.fetch_item(_db, cur.lastrowid)
    ...
    async def patch_item(item_id: int, request: Request,
        ...
        if row is None:
            raise HTTPException(status_code=404, detail="Not Found")
        item, spawned = _apply_item_patch(_db, row, patch)
